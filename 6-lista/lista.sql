...
--post 34
update postreacao
set reacao = (select reacao.codigo from reacao where reacao.nome like '%amei%')
from reacao
where postreacao.reacao = reacao.codigo and postreacao.post = (select post from postreacao where usuario like '%pxramos@mymail.com%' order by post desc limit 1);
...
--ultra-fã
update membro set selo = 1, dataregist = datetime('now', 'localtime') where membro.usuario in (
...
--super-fã
update membro set selo = 2, dataregist = datetime('now', 'localtime') where membro.usuario in (
    select usuario.email from usuario
        join postreacao on usuario.email = postreacao.usuario
        join reacao on postreacao.reacao = reacao.codigo
    where 
...
--fã
update membro set selo = 3, dataregist = datetime('now', 'localtime') where membro.usuario in (
    select usuario.email from usuario
        join postreacao on usuario.email = postreacao.usuario
        join reacao on postreacao.reacao = reacao.codigo
    where 
